...
public class SpectateCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 1) {
			...
			List<List<UUID>> players = Arrays.asList(duel.getFirst().stream().collect(Collectors.toList()), duel.getSecond().stream().collect(Collectors.toList()));
			...
			if (!duel.getSpectator().isEmpty()) {
				duel.getSpectator().forEach(spectator -> {
					if (profile.getSpectateSettings().get(0).booleanValue()) Bukkit.getPlayer(sender.getName()).showPlayer(Bukkit.getPlayer(spectator));
					if (!profile.getSpectateSettings().get(0).booleanValue()) Bukkit.getPlayer(sender.getName()).hidePlayer(Bukkit.getPlayer(spectator));
				});
			}
			if (profile.getSpectateSettings().get(1).booleanValue()) Bukkit.getPlayer(sender.getName()).setFlySpeed(0.1f);
			if (!profile.getSpectateSettings().get(1).booleanValue()) Bukkit.getPlayer(sender.getName()).setFlySpeed(0.25f);
